...
**Do not use when:** the user is asking about the log contents (resolve path from `~/.config/clog/config.yaml` → `log_root/log_subdir/YYYYMMDD.jsonl`, then `cat` directly), or doing a retroactive sweep for missed entries (use [`clog-sweep`](../clog-sweep/SKILL.md)).
...
**Finding the binary:** `clog` must be on PATH. If not, set `$CLOG_CLI_PATH` to the full path, or check `cli_path` in `~/.config/clog/config.yaml`.
...
Each entry is a JSONL line written to `$LOG_DIR/YYYYMMDD.jsonl`, where `LOG_DIR` is `log_root/log_subdir` from `~/.config/clog/config.yaml`.
